...
$(VENDOR_SDK_DIR_3.0.4)/.dir:
	...
	git clone https://github.com/espressif/ESP8266_NONOS_SDK $(VENDOR_SDK_DIR_3.0.4)
	(cd $(VENDOR_SDK_DIR_3.0.4); git checkout tags/v3.0.4)
	touch $@

$(VENDOR_SDK_DIR_2.2.1)/.dir:
	echo $(VENDOR_SDK_DIR_2.2.1)
	git clone https://github.com/espressif/ESP8266_NONOS_SDK $(VENDOR_SDK_DIR_2.2.1)
	(cd $(VENDOR_SDK_DIR_2.2.1); git checkout tags/v2.2.1)
	touch $@
...
.sdk_patch_3.0.4: user_rf_cal_sector_set.o
	...
	$(PATCH) -d $(VENDOR_SDK_DIR) -p1 < c_types-c99_sdk_304.patch
...
.sdk_patch_2.2.1: user_rf_cal_sector_set.o
	...
	$(PATCH) -d $(VENDOR_SDK_DIR) -p1 < c_types-c99_sdk_221.patch
	cd $(VENDOR_SDK_DIR)/lib; mkdir -p tmp; cd tmp; $(TOOLCHAIN)/bin/xtensa-lx106-elf-ar x ../libcrypto.a; cd ..; $(TOOLCHAIN)/bin/xtensa-lx106-elf-ar rs libwpa.a tmp/*.o
	$(TOOLCHAIN)/bin/xtensa-lx106-elf-ar r $(VENDOR_SDK_DIR)/lib/libmain.a user_rf_cal_sector_set.o
	@touch $@
